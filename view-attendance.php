...
//echo "hi";
  $select =  mysql_query("select * from attendance where date = '".$_POST['date']."' AND dep = '".$_POST['dept']."' AND sem = '".$_POST['sem']."' ",$con);
  //echo "hello";
  //$row = mysql_num_rows($select,$con);
while($row=mysql_fetch_array($select))
{
  $stt=$row['stud_id'];
  $att=unserialize($stt);
  foreach ($att as $key => $value) {
    $select1 =  mysql_query("select * from students where id= '".$value."' ",$con);
    $rows=mysql_fetch_array($select1);
